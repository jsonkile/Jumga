...
class FashionFragment : Fragment() {
    ...
    private fun setupUI() {
        requireActivity().findViewById<MaterialToolbar>(R.id.main_toolbar)
            .setTitle(R.string.fashion_fragment_heading)

        recyclerView = fashion_recycler_view
        ...
        productsAdapter.setOnItemClickListener(object : ProductsAdapter.ClickListener {
            override fun onClick(view: View, pos: Int, product: Product) {
                val intent =
                    Intent(this@FashionFragment.requireActivity(), ProductActivity::class.java)
                val bundle = Bundle()
        ...
        fashion_page_refresh?.setOnRefreshListener {
            getProducts()
        }
    }

    private fun getProducts() {
        fashion_page_refresh?.isRefreshing = true
        db.collection("products")
            .whereEqualTo("category", 1)
            .get()
            .addOnCompleteListener {
                fashion_page_refresh?.isRefreshing = false
            }

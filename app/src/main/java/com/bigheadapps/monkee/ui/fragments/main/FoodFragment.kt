...
class FoodFragment : Fragment() {
    ...
    private fun setupUI() {
        requireActivity().findViewById<MaterialToolbar>(R.id.main_toolbar)
            .setTitle(R.string.food_fragment_heading)
        ...
        productsAdapter.setOnItemClickListener(object : ProductsAdapter.ClickListener {
            override fun onClick(view: View, pos: Int, product: Product) {
                val intent =
                    Intent(this@FoodFragment.requireActivity(), ProductActivity::class.java)
                val bundle = Bundle()
        ...
        food_page_refresh?.setOnRefreshListener {
            getProducts()
        }
    }

    private fun getProducts() {
        food_page_refresh?.isRefreshing = true
        db.collection("products")
            .whereEqualTo("category", 0)
            .get()
            .addOnCompleteListener {
                food_page_refresh?.isRefreshing = false
            }
